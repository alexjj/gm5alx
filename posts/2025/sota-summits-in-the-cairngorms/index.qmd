...
map = {
  ...
  let map = L.map(container).setView([57.07, -3.7], 11.5); // initializes the map, sets zoom & coordinates
  ...
  // Define a color scale for summit points
  const colorScale = d3.scaleOrdinal(d3.schemeCategory10);

  // Add summit markers to the map
  summits.forEach(({ latitude, longitude, points, name }) => {
    L.circleMarker([+latitude, +longitude], {
      radius: 6,
      color: colorScale(points),
      fillOpacity: 0.8,
    })
      .bindPopup(`Summit: ${name}<br>Points: ${points}`)
      .addTo(map);
  });
}
```
