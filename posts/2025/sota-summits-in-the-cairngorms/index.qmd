...
summits = csv(FileAttachment("cairngorms_summits.csv"))

// Create a map and add a tile layer
map = {
  const map = L.map(DOM.element("div", { style: "height: 600px" })).setView([57.12, -3.64], 10);
  ...
  // Create a color scale for the points column
  const colorScale = d3.scaleOrdinal(d3.schemeCategory10);

  // Add summits as markers
  summits.forEach((summit) => {
    const { latitude, longitude, points } = summit;
    L.circleMarker([+latitude, +longitude], {
      radius: 6,
      color: colorScale(points),
      fillOpacity: 0.8,
    })
      .bindPopup(`Summit: ${summit.name}<br>Points: ${points}`)
      .addTo(map);
  });
